...
function createStylisInstance(options, plugins) {
  if (options === void 0) {
  ...
  var stylis = new Stylis(_extends({
    global: false,
    cascade: true,
    keyframe: false,
    prefix: true,
...
function (_Error) {
  ...
  function StyledComponentsError(code) {
    ...
    if (process.env.NODE_ENV === 'production') {
      _this = _Error.call(this, "An error occurred. See https://github.com/styled-components/styled-components/blob/master/packages/styled-components/src/utils/errors.md#" + code + " for more information." + (interpolations.length > 0 ? " Additional arguments: " + interpolations.join(', ') : '')) || this;
    } else {
  ...
  _proto.generateAndInjectStyles = function generateAndInjectStyles(executionContext, styleSheet) {
    ...
    if (this.isStatic) {
      if (!styleSheet.hasNameForId(componentId, componentId)) {
        ...
        var cssStaticFormatted = styleSheet.options.stringifier(cssStatic, "." + componentId, undefined, componentId);
      ...
      if (!styleSheet.hasNameForId(componentId, name)) {
        var cssFormatted = styleSheet.options.stringifier(css, "." + name, undefined, componentId);
...
function StyleSheetManager(props) {
  ...
  var _useState = React.useState({
    ...
    stylisPlugins: props.stylisPlugins
      ...
      stylisOptions = _useState$.stylisOptions,
      stylisPlugins = _useState$.stylisPlugins;

  if (process.env.NODE_ENV !== 'production') {
    if (!shallowequal(stylisOptions, props.stylisOptions)) {
      // eslint-disable-next-line no-console
      console.warn('stylisOptions are frozen on initial mount of StyleSheetManager. Changing this prop dynamically will have no effect.');
    }
...
function useStyledComponentImpl(forwardedComponent, props, forwardedRef) {
  ...
  var generatedClassName = useInjectedStyle(componentStyle, componentAttrs.length > 0, context, utils, process.env.NODE_ENV !== 'production' ? forwardedComponent.warnTooManyClasses : undefined);
