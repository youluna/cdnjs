...
var SC_VERSION = "5.0.0-0.canary-sheet";
...
function stringifyRules(rules, selector, prefix, componentId) {
...
function () {
  function ComponentStyle(rules, attrs, componentId) {
    ...
    this.componentId = componentId; // NOTE: This registers the componentId, which ensures a consistent order
  ...
  _proto.generateAndInjectStyles = function generateAndInjectStyles(executionContext, styleSheet) {
    ...
    if (isStatic && prevName !== undefined && styleSheet.hasNameForId(componentId, prevName)) {
      return prevName;
    }

    var flatRules = flatten(this.rules, executionContext, styleSheet);
    var name = hasher(this.componentId + flatRules.join(''));

    if (!styleSheet.hasNameForId(componentId, name)) {
      var css = stringifyRules(flatRules, "." + name, undefined, componentId);
...
function keyframes(strings) {
  ...
  var rules = css.apply(void 0, [strings].concat(interpolations));
  var name = hasher(replaceWhitespace(JSON.stringify(rules)));
