(function (global, factory) {
  ...
  function decomposeString(fromValue) {
    ...
    var value = typeof fromValue !== 'string' ? fromValue : fromValue.replace(hexColor, hex2rgb).match(NUM_REGEX).map(function (v) {
  ...
  function decompose(prop, obj, from, to, stringBuffer) {
    ...
    if (typeof fromValue === 'number' && typeof toValue === 'number') ; else if (fromValue && fromValue.splice && fromValue.isString && toValue && toValue.splice && toValue.isString) ; else if (typeof fromValue === 'string' && Array.isArray(toValue)) {
      ...
      while (i < _fromValue.length) {
        if (_fromValue[i] === toValue1[i] && typeof _fromValue[i - 1] === 'string') {
          _fromValue.splice(i - 1, 2, _fromValue[i - 1] + _fromValue[i]);
