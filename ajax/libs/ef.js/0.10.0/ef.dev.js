(function (global, factory) {
	...
	var createByTag = function (tag, svg, custom) {
	...
	var getElement = function (ref$1) {
		...
		var refs = ref$1.refs;
		var svg = ref$1.svg;
		var custom = ref$1.custom;

		var element = createByTag(tag, svg, custom);
	...
	var addValListener = function (ref) {
		...
		if (key === 'value') {
			...
			element[addListener]('ef-change-event', function () { return updateOthers({parentNode: parentNode, handlerNode: handlerNode, _handler: _handler, _key: _key, value: element.checked}); });
		}
	};
	...
	var addProp = function (ref) {
		...
		var lastNode = resolveReactivePath(keyPath, element);
		if (typeValid(prop)) {
			...
			if ((key === 'value' ||
				key === 'checked') &&
				!prop[0]) { addValListener({_handler: _handler, ctx: ctx, handlers: handlers, subscribers: subscribers, innerData: innerData, element: element, key: key, expr: prop[1], custom: custom}); }
			queue([_handler]);
		}
	};
	...
	var createElement = function (ref) {
		...
		var element = getElement({tag: tag, ref: r, refs: refs, svg: svg, custom: custom});
		for (var key in a) { addAttr({element: element, custom: custom, attr: a[key], key: key, ctx: ctx, handlers: handlers, subscribers: subscribers, innerData: innerData}); }
		for (var key$1 in p) { addProp({element: element, custom: custom, prop: p[key$1], key: key$1, ctx: ctx, handlers: handlers, subscribers: subscribers, innerData: innerData}); }
		for (var key$2 in e) { addEvent({element: element, custom: custom, event: e[key$2], ctx: ctx, handlers: handlers, subscribers: subscribers, innerData: innerData}); }
		return element
	};
	...
	var EFBaseComponent = /*@__PURE__*/(function () {
		...
		EFBaseComponent.prototype.$emit = function $emit (event, options) {
			{ checkDestroyed(this); }
		...
		EFBaseComponent.prototype.$on = function $on () {
			...
			while ( len-- ) args[ len ] = arguments[ len ];
			{ checkDestroyed(this); }
			(ref = this.$ctx.nodeInfo.placeholder).addEventListener.apply(ref, args);
		};
		...
		EFBaseComponent.prototype.$off = function $off () {
			...
			while ( len-- ) args[ len ] = arguments[ len ];
			{ checkDestroyed(this); }
				(ref = this.$ctx.nodeInfo.placeholder).removeEventListener.apply(ref, args);
		};
	...
	var create$1 = function (value) {
		...
		var EFComponent = /*@__PURE__*/(function (EFBaseComponent) {
			function EFComponent(newState, scope) {
				inform();
