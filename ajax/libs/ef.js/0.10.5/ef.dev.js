(function (global, factory) {
	...
	var regTmpl = function (ref) {
		...
		if (Array.isArray(val)) {
			...
			var _handler = function () {
				...
				handling = true;
				inform();
				handler(mixVal.apply(void 0, tmpl));
				exec();
				handling = false;
			};
	...
	var addProp = function (ref) {
		...
		if (typeValid(prop)) {
			...
			if ((key === 'value' ||
				key === 'checked') &&
				!prop[0]) { addValListener({ctx: ctx, handlers: handlers, subscribers: subscribers, innerData: innerData, element: element, key: key, expr: prop[1], custom: custom}); }
			queue([_handler]);
		}
	};
