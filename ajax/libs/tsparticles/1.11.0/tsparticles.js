...
Constants.touchStartEvent = "mousestart";
...
class Utils_Utils {
  ...
  static itemFromArray(array, index) {
    return array[index !== undefined ? index : Utils_Utils.arrayRandomIndex(array)];
  }
...
class ColorUtils_ColorUtils {
  ...
  static getStyleFromColor(color, opacity) {
    const opacityValue = opacity !== null && opacity !== void 0 ? opacity : 0.4;
...
class ShapeUtils_ShapeUtils {
  ...
  static drawLineShape(context, length, stroke, strokeColor) {
    if (strokeColor) {
      ...
      context.strokeStyle = ColorUtils_ColorUtils.getStyleFromColor(strokeColor);
...
class CanvasUtils_CanvasUtils {
  ...
  static gradient(context, p1, p2, midColor) {
    ...
    grad.addColorStop(0, ColorUtils_ColorUtils.getStyleFromColor(p1.color));
    grad.addColorStop(gradStop > 1 ? 1 : gradStop, midColor);
    grad.addColorStop(1, ColorUtils_ColorUtils.getStyleFromColor(p2.color));
    return grad;
  }
  ...
  static drawParticle(context, particle, colorValue, backgroundMask, radius) {
    context.save();
    ...
    ShapeUtils_ShapeUtils.drawShape(context, particle, radius, stroke, strokeColor);
    context.closePath();

    if (stroke.width > 0 && particle.strokeColor) {
      context.strokeStyle = ColorUtils_ColorUtils.getStyleFromColor(particle.strokeColor);
      context.lineWidth = stroke.width;
      context.stroke();
    }
...
class Canvas_Canvas {
  ...
  isPointInside(point, radius) {
    return this.areBoundsInside(this.calculateBounds(point, radius !== null && radius !== void 0 ? radius : 0));
  }

  areBoundsInside(bounds) {
    return bounds.left >= 0 && bounds.right <= this.dimension.width && bounds.top >= 0 && bounds.bottom <= this.dimension.height;
  }

  calculateBounds(point, radius) {
    return {
  ...
  drawParticle(particle) {
    ...
    CanvasUtils_CanvasUtils.drawParticle(this.context, particle, colorValue, options.backgroundMask.enable, radius);
  }
  ...
  lineStyle(p1, p2) {
    if (p1.color && p2.color) {
      ...
      const midColor = ColorUtils_ColorUtils.getStyleFromColor(rgb);

      if (this.context) {
        return CanvasUtils_CanvasUtils.gradient(this.context, p1, p2, midColor);
      }
    }
  }
...
var EventListeners_awaiter = undefined && undefined.__awaiter || function (thisArg, _arguments, P, generator) {
  function adopt(value) {
    return value instanceof P ? value : new P(function (resolve) {
      resolve(value);
    });
...
class EventListeners_EventListeners {
  ...
  handleWindowResize() {
    ...
    if (options.polygon.enable && options.polygon.type !== PolygonMaskType.none) {
      ...
      container.polygon.redrawTimeout = setTimeout(() => EventListeners_awaiter(this, void 0, void 0, function* () {
        container.polygon.raw = yield container.polygon.parseSvgPathToPolygon();
        container.particles.clear();
        container.particles.init();
        container.particles.draw(0);
      }), 250);
...
class Particle_Particle {
  constructor(container, position) {
    ...
    if (options.particles.shape.stroke instanceof Array) {
      this.stroke = Utils_Utils.itemFromArray(options.particles.shape.stroke);
    } else {
...
class Particles_Particles {
  ...
  init() {
    ...
    if (options.polygon.enable && options.polygon.type === PolygonMaskType.inline && options.polygon.inline.arrangement === PolygonMaskInlineArrangement.onePerPoint) {
      container.polygon.drawPointsOnPolygonPath();
    } else {
      for (let i = this.array.length; i < options.particles.number.value; i++) {
        const p = new Particle_Particle(container);
        this.array.push(p);
      }
    }
  ...
  push(nb, mousePosition) {
    ...
    for (let i = 0; i < nb; i++) {
      const p = new Particle_Particle(container, pos);
      this.addParticle(p);
    }
  ...
  drawPointsOnPolygonPath() {
    ...
    if (this.raw) {
      for (const item of this.raw) {
        ...
        const particle = new Particle_Particle(container, position);
...
class FrameManager_FrameManager {
  constructor(container) {
    this.container = container;
  }
...
class Events_Events {
  constructor() {
    this.onClick = new ClickEvent_ClickEvent();
...
// CONCATENATED MODULE: ./dist/Classes/Options/Particles/Shape/Stroke.js
...
// CONCATENATED MODULE: ./dist/Classes/Options/Particles/ParticlesSizeAnimation.js
class ParticlesSizeAnimation {
  constructor() {
    this.enable = false;
    this.minimumValue = 0;
    this.speed = 5;
...
// CONCATENATED MODULE: ./dist/Classes/Options/Particles/ParticlesSize.js


class ParticlesSize_ParticlesSize {
  constructor() {
...
class Particles_Particles_Particles {
  constructor() {
    ...
    this.size = new ParticlesSize_ParticlesSize();
  ...
  load(data) {
    ...
    if (data !== undefined) {
      if (data.color !== undefined) {
        if (typeof this.color === "string") {
          this.color = data.color;
          this.opacity = (_a = ColorUtils_ColorUtils.stringToAlpha(data.color)) !== null && _a !== void 0 ? _a : this.opacity;
        } else {
          this.color.load(data.color);
        }
      }
...
// CONCATENATED MODULE: ./dist/Enums/PresetType.js
var PresetType;

(function (PresetType) {
  PresetType["basic"] = "basic";
  PresetType["backgroundMask"] = "backgroundMask";
  PresetType["fontAwesome"] = "fontAwesome";
  PresetType["snow"] = "snow";
  PresetType["bouncing"] = "bouncing";
  PresetType["stars"] = "stars";
...
class Options_Options {
  ...
  importPreset(preset) {
    const presetOptions = Presets_Presets.getPreset(preset);
...
class Container_Container {
  constructor(id, params, ...presets) {
    ...
    this.drawer = new FrameManager_FrameManager(this);
    this.interactivity = {
      mouse: {}
    };
    this.images = [];
    ...
    for (const preset of presets) {
      this.options.load(Presets_Presets.getPreset(preset));
    }
  ...
  start() {
    return Container_awaiter(this, void 0, void 0, function* () {
      ...
      if (this.options.polygon.enable && this.options.polygon.url) {
        this.polygon.raw = yield this.polygon.parseSvgPathToPolygon(this.options.polygon.url);
      }
  ...
  loadImageShape(imageShape) {
    return Container_awaiter(this, void 0, void 0, function* () {
      ...
      yield this.loadImage(image, imageShape);
