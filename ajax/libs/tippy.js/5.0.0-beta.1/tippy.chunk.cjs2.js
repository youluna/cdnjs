...
var __chunk_1 = require('./tippy.chunk.cjs.js');
...
function validateExtraPropsFunctionality(instance, partialProps) {
  ...
  extraProps.forEach(function (prop) {
    if (__chunk_1.hasOwnProperty(partialProps, prop) && !instance.__extraProps[prop]) {
...
function createTippy(reference, collectionProps) {
  var props = __chunk_1.evaluateProps(reference, collectionProps); // If the reference shouldn't have multiple tippys, return null early
  ...
  var listeners = [];
  var debouncedOnMouseMove = __chunk_1.debounce(onMouseMove, props.interactiveDebounce);
  /* ======================= 🔑 Public members 🔑 ======================= */

  var id = idCounter++;
  var popper = __chunk_1.createPopperElement(id, props);
  var popperChildren = __chunk_1.getChildren(popper);
  ...
  function onDocumentMouseDown(event) {
    ...
    if (getEventListenersTarget().contains(event.target)) {
      if (__chunk_1.currentInput.isTouch) {
        return;
      }

      if (instance.state.isVisible && __chunk_1.includes(instance.props.trigger, 'click')) {
        return;
      }
    }
  ...
  function onTransitionEnd(duration, callback) {
    ...
    function listener(event) {
      if (event.target === tooltip) {
        __chunk_1.updateTransitionEndListener(tooltip, 'remove', listener);
        callback();
      }
    ...
    __chunk_1.updateTransitionEndListener(tooltip, 'remove', currentTransitionEndListener);
    __chunk_1.updateTransitionEndListener(tooltip, 'add', listener);
  ...
  function addTriggersToEventListenersTarget() {
    if (getIsCustomTouchBehavior()) {
      on('touchstart', onTrigger, __chunk_1.PASSIVE);
      on('touchend', onMouseLeave, __chunk_1.PASSIVE);
    } // `click` for keyboard. Mouse uses `mousedown` (onDocumentMouseDown)


    if (!__chunk_1.includes(instance.props.trigger, 'click')) {
      on('click', function () {
        if (!__chunk_1.currentInput.isTouch && instance.props.hideOnClick === true) {
          instance.hide();
        }
      });
    }
    ...
    instance.props.trigger.trim().split(' ').forEach(function (eventType) {
      ...
      switch (eventType) {
        ...
        case 'focus':
          on(__chunk_1.isIE ? 'focusout' : 'blur', onBlur);
          break;
      }
    });
  }
  ...
  function onTrigger(event) {
    ...
    if (event.type === 'click' && instance.props.hideOnClick !== false && instance.state.isVisible) {
      ...
      if (__chunk_1.currentInput.isTouch && value === 'hold' && duration) {
  ...
  function onMouseMove(event) {
    var isCursorOverReferenceOrPopper = __chunk_1.closestCallback(event.target, function (el) {
    ...
    if (__chunk_1.isCursorOutsideInteractiveBorder(__chunk_1.getBasePlacement(instance.state.currentPlacement), popper.getBoundingClientRect(), event, instance.props)) {
  ...
  function isEventListenerStopped(event) {
    ...
    var isTouchEvent = __chunk_1.includes(event.type, 'touch');
    var isCustomTouch = getIsCustomTouchBehavior();
    return supportsTouch && __chunk_1.currentInput.isTouch && isCustomTouch && !isTouchEvent || __chunk_1.currentInput.isTouch && !isCustomTouch && isTouchEvent;
  }
  ...
  function createPopperInstance() {
    ...
    function applyMutations(data) {
      ...
      if (instance.props.flip && !instance.props.flipOnUpdate) {
        ...
        __chunk_1.setFlipModifierEnabled(instance.popperInstance.modifiers, false);
      }
    ...
    var config = __chunk_1._extends({
      ...
      modifiers: __chunk_1._extends({}, popperOptions && popperOptions.modifiers, {
        preventOverflow: __chunk_1._extends({
          ...
          padding: __chunk_1.PREVENT_OVERFLOW_PADDING
        ...
        arrow: __chunk_1._extends({
          element: arrow,
          enabled: !!arrow
        }, __chunk_1.getModifier(popperOptions, 'arrow')),
        flip: __chunk_1._extends({
          enabled: instance.props.flip,
          padding: instance.props.distance + __chunk_1.PREVENT_OVERFLOW_PADDING,
          behavior: instance.props.flipBehavior
        }, __chunk_1.getModifier(popperOptions, 'flip')),
        offset: __chunk_1._extends({
          offset: instance.props.offset
        }, __chunk_1.getModifier(popperOptions, 'offset'))
      }),
      onCreate: function onCreate(data) {
        applyMutations(data);
        __chunk_1.preserveInvocation(popperOptions && popperOptions.onCreate, config.onCreate, [data]);
        runMountCallback();
      },
      onUpdate: function onUpdate(data) {
        applyMutations(data);
        __chunk_1.preserveInvocation(popperOptions && popperOptions.onUpdate, config.onUpdate, [data]);
        runMountCallback();
      }
    });
  ...
  function mount() {
    ...
    var parentNode = appendTo === 'parent' ? reference.parentNode : __chunk_1.invokeWithArgsOrReturn(appendTo, [reference]); // The popper element needs to exist on the DOM before its position can be
    ...
    if (instance.popperInstance) {
      __chunk_1.setFlipModifierEnabled(instance.popperInstance.modifiers, instance.props.flip);
  ...
  function setProps(_ref2) {
    ...
    nextProps.ignoreAttributes = __chunk_1.hasOwnProperty(partialProps, 'ignoreAttributes') ? partialProps.ignoreAttributes || false : prevProps.ignoreAttributes;
    instance.props = nextProps;
    addTriggersToEventListenersTarget();
    cleanupInteractiveMouseListeners();
    debouncedOnMouseMove = __chunk_1.debounce(onMouseMove, nextProps.interactiveDebounce);
    __chunk_1.updatePopperElement(popper, prevProps, nextProps, instance.state.isVisible);
    instance.popperChildren = __chunk_1.getChildren(popper);

    if (instance.popperInstance) {
      if (__chunk_1.POPPER_INSTANCE_DEPENDENCIES.some(function (prop) {
        return __chunk_1.hasOwnProperty(partialProps, prop) && partialProps[prop] !== prevProps[prop];
      })) {
  ...
  function show(duration, shouldPreventPopperTransition) {
    ...
    var isTouchAndTouchDisabled = __chunk_1.currentInput.isTouch && !instance.props.touch;
    ...
    __chunk_1.setTransitionDuration(shouldPreventPopperTransition ? transitionableElements.concat(popper) : transitionableElements, 0);
    ...
    currentMountCallback = function currentMountCallback() {
      ...
      __chunk_1.setTransitionDuration([popper], instance.props.updateDuration);
      __chunk_1.setTransitionDuration(transitionableElements, duration);
      __chunk_1.setVisibilityState(transitionableElements, 'visible');
  ...
  function hide(duration) {
    ...
    var transitionableElements = getTransitionableElements();
    __chunk_1.setTransitionDuration(transitionableElements, duration);
    __chunk_1.setVisibilityState(transitionableElements, 'hidden');
...
function tippy(targets, optionalProps) {
  ...
  __chunk_1.bindGlobalEventListeners();

  var props = __chunk_1._extends({}, __chunk_1.defaultProps, {}, optionalProps);

  var elements = __chunk_1.getArrayOfElements(targets);

  if (process.env.NODE_ENV !== "production") {
    var isSingleContentElement = __chunk_1.isRealElement(props.content);
  ...
  return __chunk_1.isRealElement(targets) ? instances[0] : instances;
}

tippy.version = version;
tippy.defaultProps = __chunk_1.defaultProps;
tippy.currentInput = __chunk_1.currentInput;
/**
...
tippy.setDefaultProps = function (partialProps) {
  ...
  Object.keys(partialProps).forEach(function (key) {
    // @ts-ignore
    __chunk_1.defaultProps[key] = partialProps[key];
  });
};
/**
...
tippy.hideAll = function (_temp) {
  ...
  __chunk_1.arrayFrom(document.querySelectorAll(__chunk_1.POPPER_SELECTOR)).forEach(function (popper) {
    ...
    if (instance) {
      ...
      if (excludedReferenceOrInstance) {
        isExcluded = __chunk_1.isReferenceElement(excludedReferenceOrInstance) ? instance.reference === excludedReferenceOrInstance : popper === excludedReferenceOrInstance.popper;
      }
...
function autoInit() {
  __chunk_1.arrayFrom(document.querySelectorAll('[data-tippy]')).forEach(function (el) {
