...
$__System.register("1", ["2"], function (_export, _context5) {
	...
	var CustomEvent, _regeneratorRuntime, _asyncToGenerator, Loader, NOT_LOADED, LOADING_SOURCE_CODE, NOT_BOOTSTRAPPED, BOOTSTRAPPING, NOT_MOUNTED, MOUNTING, MOUNTED, UNMOUNTING, SKIP_BECAUSE_BROKEN, _extends, globalTimeoutConfig, toLoadPromise, toBootstrapPromise, toMountPromise, toUnmountPromise, capturedEventListeners, routingEventsListeningTo, originalAddEventListener, originalRemoveEventListener, originalPushState, originalReplaceState, hasInitialized, childApps, appChangeUnderway, peopleWaitingOnAppChange, started;
	...
	function transformErr(ogErr, childApp) {
		var errPrefix = "'" + childApp.appLocation + "' died in status " + childApp.status + ": ";
  ...
  */
	...
	function getAppStatus(appName) {
		var app = find(childApps, function (app) {
			return app.appLocation === appName;
		});
		return app ? app.status : null;
	}

	function declareChildApplication(appLocation, activeWhen) {
		if (!Loader) throw new Error('You cannot declare a single-spa child application without first specifying the single-spa Loader. Please call singleSpa.setLoader API before declaring child applications');
		...
		if (childApps[appLocation]) throw new Error('There is already an app declared at location ' + appLocation);
						...
						while (1) {
							switch (_context.prev = _context.next) {
								...
								case 2:
									...
									_context.prev = 4;
									_context.next = 7;
									return Loader.import(app.appLocation);
						...
						while (1) {
							switch (_context.prev = _context.next) {
								...
								case 2:
									...
									_context.prev = 3;
									_context.next = 6;
									return reasonableTime(app.bootstrap(), 'Bootstrapping app \'' + app.appLocation + '\'', app.timeouts.bootstrap);
						...
						while (1) {
							switch (_context.prev = _context.next) {
								...
								case 2:
									_context.prev = 2;
									_context.next = 5;
									return reasonableTime(app.mount(), 'Mounting application ' + app.appLocation + '\'', app.timeouts.mount);
						...
						while (1) {
							switch (_context.prev = _context.next) {
								...
								case 2:
									...
									_context.prev = 3;
									_context.next = 6;
									return reasonableTime(app.unmount(), 'Unmounting application ' + app.appLocation + '\'', app.timeouts.unmount);
